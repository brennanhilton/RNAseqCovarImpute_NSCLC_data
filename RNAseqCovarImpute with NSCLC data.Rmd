...
Data processing
Data come from EMBL-EBI: E-GEOD-81089.
```{r}
data <- read_tsv("./E-GEOD-81089-experiment-design.tsv") %>% janitor::clean_names()
colnames(data) <- gsub("^sample_characteristic_", "", colnames(data))
# Select the covariates we want
data <- data %>% dplyr::select(run, sex, age, clinical_information, sampling_site)
# Make age numeric instead of "60 year", "70 year", etc.
...
```{r}
# Multivariate model examines effects of sex, age, and smoking status on gene expression
...
eout <- eBayes(lout)
# Get the significant DEGs for male versus female sex
top.table1 <- topTable(eout, coef = "sexmale", sort.by = "P", n = Inf) %>%
  mutate(significant = ifelse(adj.P.Val < 0.05, "Yes", "No"))
# truth_res stores the 'true DEGs' from the full data model
truth_res <- tibble(
...
Set up missing patterns. Example
```{r}
adj_formula <- "~sex + age + smoking"

# setup missing data patterns. See mice::ampute for more details on the missing patterns and weights
...
# Run PCA
p <- pca(pca_data)
# Horn's parallel analysis determines the number of PCs to retain
# Retains PCs that explain more variation than PCs from random data
...
Make sure to set the proper number of cores according to your own environment.
```{r}
library(tidyverse)
library(Amelia)
library(mice)
...
simulation2 <- function(data, mat, voom_formula, predictor, miss_mech, prop_miss, pattern, pcs) {
  RNGkind("L'Ecuyer-CMRG") # in dorng this RNGkind ensures the different RNG streams are statistically independent. We want 10 different datasets with simulated missingness
  mcoptions <- list(mc.set.seed = TRUE)
  myCluster <- makeCluster(10, # number of cores to use, can use detectCores() to see how many available to you
    type = "PSOCK"
  ) # type of cluster
  registerDoParallel(myCluster)
  # With dorng, calling set.seed before the loop ensures reproducibility of the results
  set.seed(2023)
...
If all files are saved above, you should close out and restart here to free up R environment.
```{r}
library(tidyverse)
library(Amelia)
library(mice)
